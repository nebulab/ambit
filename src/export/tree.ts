...
export function packageTree(
  ...
  const add = (name: string, entry: ExportEntry): void => {
    ...
    if (parent !== "." && !tree.has(parent)) {
      add(parent, { data: null, mode: 0o755 });
    }
  ...
  for (const item of packages) {
    ...
    for (const [relative, file] of item.files) {
      if (linkedDirectories.some((directory) => relative.startsWith(`${directory}/`))) {
        continue;
      }
      ...
      if (
        ...
        if (file.data === null) {
          linkedDirectories.push(relative);
        }
      } else {
        add(name, file);
      }
...
export async function readTree(root: string): Promise<ExportTree> {
  ...
  const visit = async (relative: string): Promise<void> => {
    ...
    if (info.isSymbolicLink()) {
      ...
      if (relative) {
        tree.set(relative, { data: null, mode: info.mode & 0o777 });
      }
