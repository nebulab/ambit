 ...
 *
 * The subject declares its namespace, and so does every `requires` and `expects` entry the command
 * writes. That is
 ...
 * declares `scopes`, `requires`, and `expects`, while an MCP entity and a hook have no `requires` at all —
 ...
 * an unresolvable requirement and there is no better advice to add. `--add-expects` is not checked
 * either, and cannot be: nothing in a catalog says whether a variable exists, which is the whole
 * difference between the list ambit resolves and the list `doctor` checks. Everything else — atomic
 * writes, the root check, `--dry-run`, and validation of the whole result — comes from the B02 editor.
 */
...
import { EXPECTS, formatExpectation } from "../model/expectation.js";
import type { ReferenceGrammar } from "../model/reference.js";
import { parseReference, parseSubject } from "../model/reference.js";
import type { ItemKind, Requirement } from "../model/requirement.js";
import { REQUIRES, formatRequirement } from "../model/requirement.js";
...
/**
 * Which annotations are lists of `<kind>:<name>` references rather than of bare strings, and by whose
 * grammar each is read back.
 *
 * A key absent here is a plain string list — `scopes`, today. Driving the writer off this table rather
 * off the key's name is what lets the loop below stay one loop: every annotation is a set of strings
 * while the arithmetic runs, and only the write knows the difference.
 */
const REFERENCE_GRAMMARS: Readonly<Partial<Record<AnnotationKey, ReferenceGrammar>>> = {
  requires: REQUIRES,
  expects: EXPECTS,
};
...
const MCP_WORDING: LeafWording = {
  noun: "MCP server",
  declares: "an entity declares `name`, `scopes`, `transport`, and `expects`",
};

const HOOK_WORDING: LeafWording = {
  noun: "hook",
  declares: "a hook declares its `event` and `command`, with `scopes` and `expects` beside them",
};
...
export function annotationSubject(name: string): Requirement {
  return parseSubject<ItemKind>(
    REQUIRES,
    name,
    `\`annotate ${name}\` does not say what to annotate`,
  );
}
...
async function subjectOf(root: string, catalog: Catalog, name: string): Promise<Subject> {
  ...
  if (target.kind === "mcp") {
    ...
    return {
      ...
      current: {
        ...
        requires: [],
        expects: entity.expects.map(formatExpectation),
      },
      leaf: MCP_WORDING,
    };
  }
  ...
  if (target.kind === "hook") {
    ...
    return {
      ...
      current: {
        scopes: hook.scopes,
        requires: [],
        expects: hook.expects.map(formatExpectation),
      },
      leaf: HOOK_WORDING,
    };
  }
  ...
  return {
    ...
    current: {
      ...
      // `requires` list sort by namespace and then by name, and an `expects` list by kind.
      requires: skill.requires.map(formatRequirement),
      expects: skill.expects.map(formatExpectation),
    },
  };
}
...
/**
 * Rejects an entry the edit names without a kind, in whichever list's grammar it belongs to.
 *
 ...
 * to say which kind the dangling entry is, since more than one could hold the name.
 *
 * Exported so the command's argv rule can refuse before Commander dispatches, in the same words.
 *
 * @throws {AmbitError} exit 2 for an entry that names no kind.
 */
export function assertReferenceRefs(
  grammar: ReferenceGrammar,
  edit: AnnotationEdit | undefined,
): void {
  for (const value of [...(edit?.add ?? []), ...(edit?.remove ?? [])]) {
    parseReference(grammar, value);
  }
}

/**
 * Adds and removes entries in a skill's, an MCP entity's or a hook's `scopes`, `requires`, and
 * `expects`.
 *
 ...
 * @throws {AmbitError} exit 2 for a subject, a `requires` entry or an `expects` entry that names no
 *   kind, a catalog that does not parse, a `requires` edit aimed at an MCP entity or a hook, or a
 *   write that fails; exit 3 for a skill, server or hook the catalog does not provide, an added scope
 *   its registry does not hold, or a result that would not validate — with nothing written.
 */
export async function annotate(
  ...
  for (const [key, grammar] of Object.entries(REFERENCE_GRAMMARS)) {
    assertReferenceRefs(grammar, options.edits?.[key as AnnotationKey]);
  }
  ...
  for (const key of subject.keys) {
    ...
    // `requires` and `expects` hold entries rather than bare strings: each declares the kind its name
    // is in, so both are written back as the one-key mappings they were read as.
    const grammar = REFERENCE_GRAMMARS[key];
    if (grammar === undefined) {
      document.setStringList(path, values);
    } else {
      document.setReferenceList(
        path,
        values.map((value) => parseReference(grammar, value)),
      );
    }
  }
