...
const ITEM_DIRNAMES: readonly string[] = [
  HOOKS_DIRNAME,
  MCPS_DIRNAME,
  PACKS_DIRNAME,
  SKILLS_DIRNAME,
...
const BLOCKS: readonly ScaffoldBlock[] = [
  ...
  },
  {
    comment: [
      ...
      "engineer needs, skills, servers and hooks together, and `ambit dump-catalog` says what that is.",
      "",
    ...
    example: {
      requires: [{ pack: `${LOCAL_CATALOG}/*` }, { skill: `${LOCAL_CATALOG}/*` }],
    },
  },
  {
...
/**
 * Scaffolds a project in `projectDir`: `ambit.yml`, and the item directories that make it a catalog
 * of its own.
 *
