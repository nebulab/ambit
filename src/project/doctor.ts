...
interface EnvDemand {
  /** One line per declarer, in a fixed order: skills, then servers, then config placeholders. */
  readonly wanted: string[];
  /** Whether a `${VAR}` reference to it survives into a file install writes. */
...
function demandOf(demands: Map<string, EnvDemand>, variable: string): EnvDemand {
  ...
  const created: EnvDemand = { wanted: [], interpolated: false };
...
function envFindings(
  ...
  return [...demands.keys()]
    ...
    .map((variable) => {
      ...
      const step =
        demand?.interpolated === true
          ? `set ${variable}, then run \`ambit install\` again so the placeholder is interpolated`
