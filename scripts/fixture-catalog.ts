 ...
 * entities at `mcps/<name>.yml`, hooks at `hooks/<name>/HOOK.yml`, and an `ambit.yml` whose
 * `catalog:` block registers the scopes — so it doubles as the subject of the dotagents compatibility
 * test (A26): everything but `skills/` is additive, and a tool that reads only skills must be
 * unbothered by it.
 *
 ...
 * are invisible to catalog parsing, which reads only `ambit.yml`, `skills/**`, `mcps/*`, and
 * `hooks/**`.
 */
export const FIXTURE_MARKER = ".ambit-fixture";

const AMBIT_YML = `# What this repo publishes as a catalog. Every scope a skill, MCP or hook declares must be
